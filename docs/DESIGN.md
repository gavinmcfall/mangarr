...
- **Rename:** configurable scheme (e.g. `{Series}/{Series} - Ch.{chapter}.cbz`). Preserves the embedded ComicInfo.xml so Kavita parses correctly regardless.
- **Idempotency:** processed series and chapters are tracked; re-runs never re-file or double-link. New chapters of an already-filed series are picked up incrementally as they arrive.
